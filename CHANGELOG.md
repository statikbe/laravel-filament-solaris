...
- Failure report download — a tracked/queued run that finishes with failures surfaces "Download failures (CSV/XLSX)" actions in its completion notification. The file is generated on click from `solaris_batch_problems` (openspout, no stored files) via a signed download route (`filament-solaris.batch-failures.download`). Toggle with `batch_tracking.attach_failure_report` (default `true`). Tracked/queued completions now also persist to the database notification (the bell) so the download links stay accessible; inline runs additionally flash a toast.
