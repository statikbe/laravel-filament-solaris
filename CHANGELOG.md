...
- `solaris:prune-batches {--days=} {--force}` command — deletes old terminal batch runs (problems cascade) to keep the tracking tables tidy. Retention is opt-in (`--days` or `batch_tracking.prune_after_days`, default `null`); deletes only `Completed`/`Failed` runs finished before the cutoff, in chunks (`batch_tracking.prune_chunk`). Schedule it yourself. `solaris_batch_problems.batch_run_id` gained a cascading foreign key.
- Failure report download — a tracked/queued run that finishes with failures surfaces "Download failures (CSV/XLSX)" actions in its completion notification. The file is generated on click from `solaris_batch_problems` (openspout, no stored files) via a signed download route (`filament-solaris.batch-failures.download`). Toggle globally with `batch_tracking.attach_failure_report` (default `true`) or per action with `->withFailureReport()` (per-action overrides config). Tracked/queued completions now also persist to the database notification (the bell) so the download links stay accessible; inline runs additionally flash a toast.
